...
function FAQItem({ question, answer, index, isOpen, onToggle, isRTL }) {
  ...
  return (
    ...
    >
      <button
        onClick={onToggle}
        className="w-full flex items-start gap-5 py-7 focus:outline-none"
      >
        {/* Number */}
        ...
        {/* Question */}
        <span className={`flex-1 font-bold text-lg md:text-xl leading-snug transition-colors duration-300 ${
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className={`pb-7 ${isRTL ? 'pr-[3.25rem]' : 'pl-[3.25rem]'}`}>
              <div className={`relative ${isRTL ? 'pr-5 border-r-2' : 'pl-5 border-l-2'} border-accent-500/30`}>
                <p className="text-[#8888A0] text-base md:text-lg leading-relaxed">
                  {answer}
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 xl:gap-20 items-start">
          ...
          <div className="lg:col-span-2 lg:sticky lg:top-24">
            ...
            <p className="text-[#6868808] text-base md:text-lg leading-relaxed text-[#66667A] mb-8">
              {isRTL ? 'لو عندك سؤال مش موجود هنا — كلمنا على واتساب وهنرد عليك في ثواني.' : 'Have a question not listed here? Message us on WhatsApp and we\'ll reply in seconds.'}
            </p>
            ...
            >
              ...
              {isRTL ? 'اسألنا على واتساب' : 'Ask us on WhatsApp'}
            </a>
          ...
          <div className="lg:col-span-3">
            {/* Top rule */}
            <div className={`h-px mb-2 ${isRTL ? 'bg-gradient-to-l' : 'bg-gradient-to-r'} from-accent-500/20 via-white/5 to-transparent`} />
